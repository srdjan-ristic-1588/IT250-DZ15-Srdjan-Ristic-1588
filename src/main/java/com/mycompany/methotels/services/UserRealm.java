...
// Null username is invalid
        Admin user = checkUser(username, getMD5Hash(new String(upToken.getPassword())));
        if (user == null) {
    ...
    private Admin checkUser(String username, String email) {
        try {
            Admin u = (Admin) session.createCriteria(Admin.class).add(Restrictions.eq("email",
                    username)).add(Restrictions.eq("password", email)).uniqueResult();
            if (u != null) {
                return u;
            }
            return null;
    ...
    private Admin findByUsername(String username) {
        List<Admin> users = session.createCriteria(Admin.class).add(Restrictions.eq("email", username)).list();
        if (users.size() > 0) {
            return users.get(0);
        } else {
            return null;
